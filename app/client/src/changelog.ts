...
export const CHANGELOG: ChangelogEntry[] = [
  {
    version: 'v0.37.0',
    date: '2026-07-08',
    image: 'dealroom-app:v64',
    revision: 'ca-dealroom-orch-dev-swc--0000058',
    title: 'Partner IC-memo workflow — approve the memo, sign off lanes, grounded &amp; cited',
    tag: 'feature',
    highlights: [
      ...
      'Added complete_lane so the owning MD (or the Analyst / Partner) can sign a diligence lane off as complete (100%). A lane cannot be completed while it still has an open high-severity (risk/negative) issue, so a “complete” lane truly means the workstream is done and unblocked. Both actions are governed by the same persona policy (approve_memo is Partner-only; complete_lane is lane-scoped for sector MDs) and are available in-app, over MCP, and to the persona agents in Teams.',
      'The IC memo is now synthesized from the live deal record — the thesis, the sourced key figures, each lane’s recorded contributions/findings and the issue log — with citations on every section, so the source-citation audit is clean (100/100) instead of generic, uncited boilerplate.',
      'Fixed supporting bugs uncovered along the way: run_step now persists durably through optimistic concurrency (its memo/compliance mutations could previously be lost to a fire-and-forget write), and issue IDs are now unique across process restarts (with resolveIssue preferring a still-open duplicate).',
      'Used the workflow to take National CineMedia from D2 to D4: completed all seven diligence lanes with grounded findings, resolved the advertiser-concentration risk, synthesized and Partner-approved a cited IC memo, cleared KYC/compliance, and advanced through the IC-readiness gate to READY with zero overrides.'
    ]
  },
  {
    version: 'v0.36.0',
