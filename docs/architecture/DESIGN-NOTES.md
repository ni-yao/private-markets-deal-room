...
- **v4 (current) — left-stack inputs + horizontal AI spine.** To cut arrow chaos, all *inbound* domains were stacked on the **left column** (Identity → External · M365 → a **compact** Market data · MCP rail), and the AI path was straightened into a single **horizontal spine**: `Container Apps → API Management (AI gateway) → Microsoft Foundry`, all on one baseline. The Data & Documents band was reordered **Fabric-first** (Fabric · Cosmos DB · AI Search · Storage). The old top-right External spoke was removed (EDGAR/M365 now feed the hub directly from the left), the five MCP providers share one OAuth-MCP rail, and Functions/observability sit in a clean bottom-right channel below the AI zone.

## Placement &amp; spacing corrections (Playwright-verified)

- **API Management sits on the horizontal AI spine.** APIM's role is the **AI gateway** (token limits, routing, telemetry for LLM calls), so it belongs on the path between the application and Microsoft Foundry — not grouped with identity. It is a standalone node in the gap between the Application and AI zones, vertically aligned with the Container Apps hub and Foundry so the flow reads straight across: `Container Apps → API Management (AI gateway) → Microsoft Foundry (route)`. The left zone is **Identity** (Entra ID + Managed ID).
- **Overlap/spacing checks are automated in the browser** (via Playwright `getBoundingClientRect`): every node-node pair, label-over-node, label-over-label, and label-over-zone-heading is checked. The v4 relayout reports **zero** overlaps, no sub-10px node gaps, and all edge labels sit in clear space (the `telemetry` label was relocated onto the clear part of the Functions→App Insights wire; minor internal AI-stack edge labels were dropped since the node names already convey them).
