...
- **v3 (hub-and-spoke)** — a **hub-and-spoke / centered-containment** layout: the Container Apps core is central, with domains grouped around it and connectors radiating in all directions. This matches how Azure reference architectures are actually drawn, while still honoring the notation/visual rules above (grouping, labeled single-ended connectors, a legend, a 3-size type scale, one focal hub, minimal crossings, flat cards).
- **v4 (left-stack inputs + horizontal AI spine)** — all *inbound* domains stacked on the **left column** (Identity → External · M365 → a **compact** Market data · MCP rail); the AI path straightened into a horizontal spine `Container Apps → API Management → Microsoft Foundry`; Data & Documents reordered **Fabric-first**; the five MCP providers share one OAuth-MCP rail.
- **v5 (current) — compact two-over-two right cluster.** To tighten the canvas and shorten the telemetry path, the right side became **two stacked pairs of columns**: **Microsoft Foundry moved to the top-right** with the **Observe** zone directly beneath it, and **API Management over Functions** in the middle gap. This yields two short, parallel horizontal flows out of the hub: the AI call `Container Apps → API Management → Foundry` (APIM's route trunk fans up to the model stack and across to AI Services), and the ops chain `Container Apps → Functions → Observe` on one baseline (`events` then `telemetry`). Removing the old far-right Observe column and the bottom-right Functions node cleared the lower-right and cut two long diagonal telemetry runs.

## Placement &amp; spacing corrections (Playwright-verified)

- **API Management fronts the AI zone as the AI gateway.** APIM's role is the **AI gateway** (token limits, routing, telemetry for LLM calls), so it sits in the gap between the Application and AI zones on the path `Container Apps → API Management → Microsoft Foundry`. In v5 the Foundry stack moved to the top-right, so APIM's `route` trunk runs up a shared vertical channel and fans to both the model stack (Foundry → agents) and **AI Services** (`extract · safety`). Functions sits directly under APIM so the ops path `Container Apps → Functions → Observe` is a short horizontal chain. The left zone is **Identity** (Entra ID + Managed ID).
- **Overlap/spacing checks are automated in the browser** (via Playwright `getBoundingClientRect`): every node-node pair, label-over-node, label-over-label, and label-over-zone-heading is checked. The v5 relayout reports **zero** overlaps and no sub-10px node gaps.
