...
- **API Management moved to front the AI tier.** APIM's role is the **AI gateway** (token limits, routing, telemetry for LLM calls), so it belongs on the path between the application and Azure AI Foundry — not grouped with identity. It now sits at the top of the AI zone with flow `Container Apps → API Management (AI gateway) → AI Foundry (route)`. The left zone was renamed **Identity** (Entra ID + Managed ID).
- **Overlap/spacing checks were automated in the browser** (via Playwright `getBoundingClientRect`): every node-node pair, label-over-node, label-over-label, and label-over-zone-heading is checked. The final layout reports **zero** overlaps and no sub-10px gaps (fixed Web/Teams at 8px and Key Vault/Microsoft 365 at 2px), and all edge labels sit in clear space (fixed "reason·embed", "AI gateway", "extract·safety", "telemetry", and "HTTPS" which had sat on nodes or the Application heading).
